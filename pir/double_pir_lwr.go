...
func (pi *DoublePIRLWR) Name() string {
	return "DoublePIR"
}

func (pi *DoublePIRLWR) PickParams(N, d, n, logq uint64) Params {
	good_p := Params{}
	found := false
...
func (pi *DoublePIRLWR) PickParamsGivenDimensions(l, m, n, logq uint64) Params {
	p := Params{
		N:    n,
		Logq: logq,
		L:    l,
...
func (pi *DoublePIRLWR) GetBW(info DBinfo, p Params) {
...
func (pi *DoublePIRLWR) Init(info DBinfo, p Params) State {
...
func (pi *DoublePIRLWR) InitCompressed(info DBinfo, p Params) (State, CompressedState) {
        seed := RandomPRGKey()
	return pi.InitCompressedSeeded(info, p, seed)
}

func (pi *DoublePIRLWR) InitCompressedSeeded(info DBinfo, p Params, seed *PRGKey) (State, CompressedState) {
        bufPrgReader = NewBufPRG(NewPRG(seed))
        return pi.Init(info, p), MakeCompressedState(seed)
}

func (pi *DoublePIRLWR) DecompressState(info DBinfo, p Params, comp CompressedState) State {
        bufPrgReader = NewBufPRG(NewPRG(comp.Seed))
        return pi.Init(info, p)
}

func (pi *DoublePIRLWR) Setup(DB *Database, shared State, p Params) (State, Msg) {
	A1 := shared.Data[0]
	A2 := shared.Data[1]
...
func (pi *DoublePIRLWR) FakeSetup(DB *Database, p Params) (State, float64) {
	info := DB.Info
...
func (pi *DoublePIRLWR) Query(i uint64, shared State, p Params, info DBinfo) (State, Msg) {
...
func (pi *DoublePIRLWR) Answer(DB *Database, query MsgSlice, server State, shared State, p Params) Msg {
	H1 := server.Data[0]
...
func (pi *DoublePIRLWR) Recover(i uint64, batch_index uint64, offline Msg, query Msg,
...
func (pi *DoublePIRLWR) Reset(DB *Database, p Params) {
	DB.Unsquish()
	DB.Data.Sub(p.P / 2)
}
